...
class KNNLM:
    ...
    def generate_answer(self, context: str, max_tokens: int = 75, uncertainty_threshold: float = 0.5) -> str:
        ...
        for _ in range(max_tokens):
            ...
            unique_token_probs = {}
            for token_id, distance in zip(knn_token_ids, distances):
                if token_id not in unique_token_probs:
                    unique_token_probs[token_id] = 1 / (distance + 1e-6) 
                else:
                    unique_token_probs[token_id] = max(unique_token_probs[token_id], 1 / (distance + 1e-6))

            total_weight = sum(unique_token_probs.values())
            for token_id in unique_token_probs:
                unique_token_probs[token_id] /= total_weight

            knn_probs = np.zeros(self.model.config.vocab_size, dtype=np.float32)
            for token_id, weight in unique_token_probs.items():
                knn_probs[token_id] += weight 
